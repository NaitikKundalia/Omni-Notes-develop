...
import com.ematrixinfotech.aksharservnotes.exceptions.checked.BackupAttachmentException;
...
public final class BackupHelper {
  ...
  public static boolean importDB(Context context, File backupDir) {
    File database = context.getDatabasePath(ConstantsBase.DATABASE_NAME);
    if (database.exists() && database.delete()) {
      return (StorageHelper.copyFile(new File(backupDir, ConstantsBase.DATABASE_NAME), database));
    }
    return false;
  }
