...
public class BitmapHelper {
  ...
  public static Bitmap getBitmapFromAttachment(Context mContext, Attachment mAttachment, int width,
    ...
    if (AttachmentsHelper.typeOf(mAttachment, ConstantsBase.MIME_TYPE_VIDEO, ConstantsBase.MIME_TYPE_IMAGE, ConstantsBase.MIME_TYPE_SKETCH)) {
      bmp = getImageBitmap(mContext, mAttachment, width, height);

    } else if (ConstantsBase.MIME_TYPE_AUDIO.equals(mAttachment.getMime_type())) {
      bmp = ThumbnailUtils.extractThumbnail(
          BitmapUtils
              .decodeSampledBitmapFromResourceMemOpt(mContext.getResources().openRawResource(R
                  .raw.play), width, height), width, height);

    } else if (ConstantsBase.MIME_TYPE_FILES.equals(mAttachment.getMime_type())) {
      if (ConstantsBase.MIME_TYPE_CONTACT_EXT.equals(FilenameUtils.getExtension(mAttachment.getName()))) {
        bmp = ThumbnailUtils.extractThumbnail(
            BitmapUtils
                .decodeSampledBitmapFromResourceMemOpt(mContext.getResources().openRawResource(R
