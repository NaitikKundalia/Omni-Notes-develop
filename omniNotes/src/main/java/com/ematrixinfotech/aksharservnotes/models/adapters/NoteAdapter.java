...
import com.ematrixinfotech.aksharservnotes.async.TextWorkerTask;
import com.ematrixinfotech.aksharservnotes.models.Attachment;
import com.ematrixinfotech.aksharservnotes.models.Note;
import com.ematrixinfotech.aksharservnotes.models.holders.NoteViewHolder;
...
public class NoteAdapter extends RecyclerView.Adapter<NoteViewHolder> {
  ...
  private long closestNoteReminder = Long.parseLong(ConstantsBase.TIMESTAMP_UNIX_EPOCH_FAR);
    ...
    try {
      if (note.isChecklist()) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
          holder.content.setText(Html.fromHtml(titleAndContent[1].toString(), Html.FROM_HTML_MODE_COMPACT));
        } else {
          holder.content.setText(Html.fromHtml(titleAndContent[1].toString()));
        }
        holder.title.setText(titleAndContent[0]);
  ...
  private void colorNote(Note note, View v, NoteViewHolder holder) {
    ...
    String colorsPref = mActivity.getSharedPreferences(Constants.PREFS_NAME, Context.MODE_MULTI_PROCESS)
        .getString("settings_colors_app", ConstantsBase.PREF_COLORS_APP_DEFAULT);
