...
import com.ematrixinfotech.aksharservnotes.models.adapters.category.CategoryViewHolder;
...
public class CategoryRecyclerViewAdapter extends RecyclerView.Adapter<CategoryViewHolder> {
  ...
  public void onBindViewHolder(@NonNull CategoryViewHolder holder, int position) {
    ...
    if (category.getColor() != null && category.getColor().length() > 0) {
      Drawable img = mActivity.getResources().getDrawable(R.drawable.ic_group_black);
  ...
  private boolean isSelected(int position) {
    ...
    String navigation = navigationTmp != null ? navigationTmpLocal
        : mActivity.getSharedPreferences(Constants.PREFS_NAME, Context.MODE_MULTI_PROCESS)
            .getString(ConstantsBase.PREF_NAVIGATION, navigationListCodes[0]);
